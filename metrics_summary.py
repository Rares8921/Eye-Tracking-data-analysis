...
def nss_metric(saliency, fix_points):
    ...
    vals = [s[int(np.clip(y * (MAP_H - 1), 0, MAP_H - 1)), int(np.clip(x * (MAP_W - 1), 0, MAP_W - 1))] for x, y in
            fix_points]
...
for rec_id in sorted(os.listdir(recordings_dir)):
    ...
    for name, m in [("fix", fix_map), ("waldo", waldo_map), ("saliency", sal_map)]:
        ...
        fig.update_layout(
            title=f"{rec_id} overlay {name}",
            width=MAP_W * 2, height=MAP_H * 2,
            margin=dict(l=0, r=0, t=30, b=0)
        )

        out_html = os.path.join(out_dir, f"{rec_id}_overlay_{name}.html")
